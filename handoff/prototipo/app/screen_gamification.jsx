/* ============================================================
   Logros · Insignias · Gamificación
   ============================================================ */
(function () {
  const Icon = window.Icon;
  const { Ring, ProgressBar, SectionHead } = window;
  ...
  function Gamification() {
    ...
    // dinámicamente desbloquear según estado
    ...
    if (db.streak >= 7) unlocked.add('racha7');
      ...
      <div className="view-in">
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.3fr', gap:16, marginBottom:16 }} className="dash-charts">
          <div className="card card-pad" style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:12, textAlign:'center', background:'linear-gradient(160deg, var(--warn-tint), transparent)' }}>
            ...
            <div className="col" style={{ gap:2 }}><span className="num" style={{ fontSize:38, fontWeight:800, lineHeight:1 }}>{db.streak}</span><span style={{ fontWeight:700, fontSize:14 }}>días de racha</span></div>
            <p className="tiny muted" style={{ margin:0 }}>Registrando tus gastos todos los días. ¡No la rompas!</p>
          </div>
          <div className="card card-pad">
            <SectionHead icon="coins" title="Meta de ahorro del mes" />
            <div className="row" style={{ gap:20, alignItems:'center' }}>
              <Ring pct={savePct} size={104} stroke={11} color="var(--lime)">
                <div className="col" style={{ alignItems:'center' }}><span className="num" style={{ fontSize:22, fontWeight:800 }}>{savePct}%</span></div>
              </Ring>
              <div className="col grow" style={{ gap:10 }}>
                <div className="col" style={{ gap:3 }}><span className="tiny muted">Ahorrado</span><span className="num" style={{ fontWeight:800, fontSize:22 }}>{window.Store.money(s.ahorroReal)} <span className="muted" style={{ fontSize:14, fontWeight:600 }}>/ {window.Store.money(s.ahorroEsperado)}</span></span></div>
                <ProgressBar pct={savePct} color="var(--lime)" height={11} />
                {extra>0 ? <span className="pill pill-good" style={{ width:'fit-content' }}>⭐ ¡{window.Store.money(extra)} extra ahorrado!</span> : <span className="tiny muted">Te faltan {window.Store.money(Math.max(0,s.ahorroEsperado-s.ahorroReal))} para tu meta 💪</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        {extra>0 && (
          <div className="card card-pad" style={{ marginBottom:16, display:'flex', gap:14, alignItems:'center', background:'linear-gradient(120deg, var(--lime-tint), transparent)', border:'1px solid color-mix(in srgb,var(--lime) 30%,transparent)' }}>
            ...
            <p style={{ margin:0, fontSize:15.5, fontWeight:600, lineHeight:1.4 }}>¡Excelente! Ahorraste <b className="num">{window.Store.money(extra)}</b> más de lo presupuestado este mes. Ganaste una estrella ⭐</p>
          </div>
        )}

        {/* badges grid */}
        <div className="card card-pad">
          <SectionHead icon="trophy" title="Insignias" sub={`${earned} de ${BADGES.length} desbloqueadas`} right={<span className="pill pill-primary">{earned}/{BADGES.length}</span>} />
            ...
            {BADGES.map(b=>{
              ...
              return (
                <div key={b.id} className="col" style={{ alignItems:'center', textAlign:'center', gap:8, padding:'20px 14px', borderRadius:16, border:'1px solid var(--border)', background: on?'var(--surface-2)':'var(--surface)', position:'relative', opacity: on?1:.55 }}>
                  ...
                  {on ? <span className="pill pill-good" style={{ position:'absolute', top:10, right:10, padding:'3px 7px' }}><Icon name="check" size={12} /></span> : <span style={{ position:'absolute', top:12, right:12, color:'var(--faint)' }}><Icon name="lock" size={14} /></span>}
                </div>
              );
            })}
          </div>
